...
Deno.serve(async (req) => {
  ...
  try {
    // Verifica admin con cliente autenticado por JWT.
    ...
    if (!topupId || !imageUrl) {
      return json({ error: 'topup_id e image_url son requeridos.' }, 400);
    }

    const rawQr = await decodeQrFromImageUrl(imageUrl);
    if (!rawQr) {
